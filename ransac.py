...
def pose_plotter(image_points,idx):
    ...
    for e in range(12):
        ...
        plt.plot(x,y,'ro-')
        plt.axis('off')
    plt.savefig("sol_2/"+image_list[idx],bbox_inches = 'tight',pad_inches = 0)
    plt.close()
...
def ransac_full(num_matches,idx,reprojectionError,iterationsCount):
    ...
    return origin_index,target_index,image_points
...
for im_number in range(0,24):
    origin_index,target_index,image_points=ransac_full(num_matches,im_number,reprojectionError,iterationsCount)
